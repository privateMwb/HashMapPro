...
#include <HashMapPro/HashMap.h>   // std::vector (BenchmarkResult/RegressionRow storage)
...
inline std::vector<BenchmarkResult> loadResults(const std::string& file) {
    ...
    for (const auto& entry : data) {
        results.push_back({entry["suite"].get<std::string>(), 
                           entry["operation"].get<std::string>(),
...
inline void printComparisonRow(std::string_view suite, std::string_view name, std::size_t iter,
    ...
    markdown_buffer() += "| " + std::string(name) 
